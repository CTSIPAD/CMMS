...
- (void)addNextSibling:(GDataXMLNode *)sible {
    ...
    if (xmlNode_ != NULL) {
        ...
        xmlNodePtr sibleNodeCopy = [sible XMLNodeCopy];
        if (sibleNodeCopy) {
            
            xmlNodePtr resultNode = xmlAddNextSibling(xmlNode_, sibleNodeCopy);
            if (resultNode == NULL) {
                
                // failed to add
                xmlFreeNode(sibleNodeCopy);
                
            } else {
                // added this child subtree successfully; see if it has
                // previously-unresolved namespace prefixes that can now be fixed up
                [[self class] fixUpNamespacesForNode:sibleNodeCopy
                                  graftingToTreeNode:xmlNode_];
            }
        }
    }
